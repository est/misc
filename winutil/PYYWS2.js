...
kp = rs(item.slice(4, 8));
mt = decode_ieee754(item.slice(8, 12))/1000;
rc = rs(item.slice(12, 14));
mc = rs(item.slice(14, 16));
lc = rs(item.slice(16, 20));
ut = rs(item.slice(20, 24));
uth = Math.floor(ut / 3600)
utm = Math.floor( (ut-uth*3600)/60 )
uts = ut - uth*3600 - utm*60
ut = uth+'h'+utm+'m'+uts+'s'
...
function post_to_pingfm(text)
{
    ...
    xhr.Send("api_key=0b48e29f10e6f46acb1d9974f32be075&user_app_key=9f21a5b1fa386c25ab4296652b9aaed3-1214056623&post_method=default&body=" + encodeURIComponent(text))
...
try
{
    ...
    {
        //WScript.Echo
        post_to_pingfm("WinOMeter yesterday for @est: key presses: "+kp+", mouse trajectory: "+mt+"m, L/M/R clicks: "+lc+"/"+mc+"/"+rc+", uptime: "+ut+" #PYYWS");
    }
